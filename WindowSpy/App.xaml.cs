...
namespace WindowSpy
{
    public partial class App : Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            // --machineid 申请码模式：只显示申请码激活窗，不进主界面
            if (e.Args.Any(a => a.Equals("--machineid", StringComparison.OrdinalIgnoreCase)))
            {
                new ActivationWindow(LicenseError.Missing, "本机申请码获取成功（把申请码发给站长）").Show();
                return;
            }

            // 授权硬门禁（放在管理员检查之前：未授权先解决授权，不弹 UAC）
            var err = LicenseManager.Validate();
            if (err != LicenseError.None)
            {
                new ActivationWindow(err).Show();
                return;
            }
