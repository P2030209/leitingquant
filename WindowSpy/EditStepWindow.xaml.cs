...
namespace WindowSpy
{
    public partial class EditStepWindow : Window
    {
        ...
        private void LoadData()
        {
            ...
            TxtQuantParam.Text = _step.QuantParam.ToString();
            TxtQuantResult.Text = _step.ResultKey;
            SelectQuantFunc(_step.QuantFunc);
            TxtMarketUrl.Text = _step.MarketUrl;
            TxtMarketPriceVar.Text = _step.PriceVar;
            TxtMarketChangeVar.Text = _step.ChangeVar;
            TxtMarketHistoryLimit.Text = _step.HistoryLimit.ToString();
            ChkSeedHistory.IsChecked = _step.SeedHistory;
            ChkAutoPick.IsChecked = _step.AutoPick;
            SelectPickStrategy(_step.PickStrategy);
            TxtBulletVar.Text = _step.BulletVar;
            TxtAiPrompt.Text = _step.AiPrompt;
            TxtAiModel.Text = _step.AiModel;
            TxtAiResult.Text = _step.ResultKey;
            TxtBanKeywords.Text = _step.BanKeywords;
            ...
            PanelQuant.Visibility = Visibility.Collapsed;
            ...
            switch (_step.Type)
            {
                ...
                case ActionType.Save:
                    ...
                    break;
                case ActionType.Quant:
                    PanelKey.Visibility = Visibility.Visible;
                    PanelQuant.Visibility = Visibility.Visible;
                    PanelDelay.Visibility = Visibility.Collapsed;
                    LblKey.Text = "源变量:";
                    break;
                case ActionType.MarketPull:
                    PanelKey.Visibility = Visibility.Visible;
                    PanelMarket.Visibility = Visibility.Visible;
                    PanelDelay.Visibility = Visibility.Collapsed;
                    LblKey.Text = "子弹名:";
                    break;
                case ActionType.AiAsk:
                    PanelAi.Visibility = Visibility.Visible;
                    PanelDelay.Visibility = Visibility.Collapsed;
                    break;
                case ActionType.BanCheck:
        ...
        private void Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (CmbQuantFunc.SelectedItem is System.Windows.Controls.ComboBoxItem qci && qci.Content is string qs && qs.Length > 0)
                _step.QuantFunc = qs.Split(' ')[0];
            if (int.TryParse(TxtQuantParam.Text, out int qp) && qp > 0) _step.QuantParam = qp;
            _step.ResultKey = _step.Type == ActionType.AiAsk ? TxtAiResult.Text : TxtQuantResult.Text;
            _step.MarketUrl = TxtMarketUrl.Text;
            _step.PriceVar = TxtMarketPriceVar.Text;
            _step.ChangeVar = TxtMarketChangeVar.Text;
            if (int.TryParse(TxtMarketHistoryLimit.Text, out int mh)) _step.HistoryLimit = mh;
            _step.SeedHistory = ChkSeedHistory.IsChecked == true;
            _step.AutoPick = ChkAutoPick.IsChecked == true;
            if (CmbPickStrategy.SelectedItem is System.Windows.Controls.ComboBoxItem pci && pci.Content is string ps && ps.Length > 0)
                _step.PickStrategy = ps.Split(' ')[0];
            _step.BulletVar = TxtBulletVar.Text;
            _step.AiPrompt = TxtAiPrompt.Text;
            _step.AiModel = string.IsNullOrWhiteSpace(TxtAiModel.Text) ? "deepseek-chat" : TxtAiModel.Text.Trim();
            _step.BanKeywords = TxtBanKeywords.Text.Trim();

            DialogResult = true;
            Close();
        }

        private void SelectQuantFunc(string func)
        {
            for (int i = 0; i < CmbQuantFunc.Items.Count; i++)
            {
                if (CmbQuantFunc.Items[i] is System.Windows.Controls.ComboBoxItem item &&
                    item.Content is string s && s.Split(' ')[0].Equals(func, StringComparison.OrdinalIgnoreCase))
                {
                    CmbQuantFunc.SelectedIndex = i;
                    return;
                }
            }
            CmbQuantFunc.SelectedIndex = 0;
        }

        private void SelectPickStrategy(string strategy)
        {
            for (int i = 0; i < CmbPickStrategy.Items.Count; i++)
            {
                if (CmbPickStrategy.Items[i] is System.Windows.Controls.ComboBoxItem item &&
                    item.Content is string s && s.Split(' ')[0].Equals(strategy, StringComparison.OrdinalIgnoreCase))
                {
                    CmbPickStrategy.SelectedIndex = i;
                    return;
                }
            }
            CmbPickStrategy.SelectedIndex = 0;
        }
