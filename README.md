...
| 功能 | 说明 |
|:---|:---|
| **🎯 裁图即用模板引擎** | 框选截图即可教会软件识别按钮，无需标注/训练/显卡；多尺度模板自动适配 100%/125%/150% DPI，毫秒级识别、内存 <5MB（基于 OpenCvSharp，Apache 2.0） |
| **📦 视觉包一键导入 + 自动坐标** | 模板打包为视觉包，换机一键导入；识别到按钮自动填坐标，新手零配置上手 |

### 📋 规划中
| 功能 | 说明 |
|:---|:---|
| **🤖 轻量 AI 识别兜底** | 半透明/动态/复杂背景等模板失效场景，由 ONNX 轻量检测器（NanoDet / PicoDet 系，Apache 2.0）自动接管，CPU 可跑 |
| **🔢 价格数字专用识别** | 针对游戏内价格字体优化数字识别，读价准确率 95% → 99%+ |
| **📦 批量识别+批量操作** | 一次识别返回屏幕上所有可操作目标，批量上架/卖出一轮清完 |
| **⚠️ 异常弹窗自动处理** | 视觉引擎实时识别「价格已变动」「库存不足」等弹窗，自动点确认或重拉行情 |
| **🧭 多分辨率自适应** | 检测坐标自动映射，1080p/2K 等主流分辨率 + 多档 DPI 可用，附校准测试工具 |

### 💡 探索中
- 🖥️ 双窗口视觉并行（A/B 窗口并行截图+识别）
- 👁️ 识别结果可视化叠加层（游戏窗口上画半透明框标出检测结果）
